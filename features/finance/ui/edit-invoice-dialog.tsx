...
export function EditInvoiceDialog({
  ...
  useEffect(() => {
    async function loadInvoiceData() {
      ...
      if (invoice.suppliers?.name) {
        const sup = suppliers.find((s) => s.name === invoice.suppliers?.name);
        if (sup) {
    ...
    loadInvoiceData();
  }, [open, invoice, supabase, form, replace, suppliers]);
  ...
  const onSubmit = async (values: InvoiceSchema) => {
    ...
    try {
      ...
      if (file) {
        ...
        const fileName = `${Date.now()}_EDIT_${Math.random()
          .toString(36)
          .substr(2, 9)}.${fileExt}`;
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-sm text-yellow-700 dark:text-yellow-400">
                      ...
                      {loadingRate ? (
                        ...
                        <span>
                          {" "}
                          Se aplicó la cotización oficial del día
                          automáticamente.
                        </span>
                      )}
                    </div>
